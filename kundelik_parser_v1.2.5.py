...
def make_links(num: int, school_num: int):
...
def make_last_date(num: int):
    """Принимает в качетсве аргумента число недель для поиска оценок и возвращает последнюю дату,
    которую запросит прорамма"""
    local_time = datetime.date.today()
    delta = datetime.timedelta(days=7)
    a = local_time - delta * num
    return a
...
def dict_results(b: list):
    """Принимает в качестве аргумента список словарей с {предмет: оценка}, и возвращает один словарь со всеми
    предметами и оценками по ним """
    subjects = {}
...
num = input('Введите количество недель, считая от текущего дня, за которое нужны оценки: ')
while 1:
    try:
        num = int(num)
    except ValueError:
        num = input('Пожалуйста, введите неотрицательное целое число: ')
    else:
        if num <= 0:
            num = input('Пожалуйста, введите неотрицательное целое число: ')
        else:
...
last_date = make_last_date(num)
print('Будут показаны оценки за период с {0}.{1}.{2} по {3}.{4}.{5}'.format(start_date.year, start_date.month,
                                                                            start_date.day, last_date.year,
